...
class ElarDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for frame_index in sample['frames']:
            ...
            pose = None
            frame_index_pose = frame_index
            if frame_index_pose > 0:
                full_path = os.path.join(sample['path'].replace('avi', 'normalisedkps'),
                                         'points_{:05d}.npy'.format(frame_index_pose))
                ...
                pose = value
                # Normalize the angle between -1 and 1 from -pi and pi - not needed for this
                # pose[:, 0] /= math.pi
            ...
            pose = pose_transform(pose).view(-1)
        ...
        pose_clip = torch.stack(pose_clip, dim=0)
        if not self.return_path:
            return (clip, pose_clip), sample['label']
        else:
            # Return sample name instead of label so we know what prediction this is 
            return (clip, pose_clip), sample['path'].split('\\')[-1][:-4]
